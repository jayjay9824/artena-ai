...
- visualConfidence ≥ 80: artist 단정. possibleCandidates는 [] 또는 보조 후보.
- visualConfidence 50~79: artist에 가장 가능성 높은 후보를 두되, possibleCandidates에 다른 후보를 함께 제시.
- visualConfidence < 50: artist=null, possibleCandidates=[]; 시각 분석에만 집중.

제목(title) 정책 — 작가보다 더 보수적으로
- title은 작가보다 한 단계 높은 확신이 필요합니다. "이 작가의 작품일 가능성이 높음"과 "정확히 이 제목의 작품임"은 다른 판단입니다.
- 정확한 제목으로 단정할 수 있는 경우에만 title을 채웁니다 (보통 visualConfidence ≥ 70 이고, 그 작가의 알려진 특정 작품이라고 명확히 식별될 때).
- 작가는 알지만 어느 작품인지 불확실하면 title=null. 일반적 묘사("untitled", "추상화", "풍경화" 등)로 채우지 않습니다.
- title을 단정할 정도의 근거가 없으면 visualReason에 "작가는 추정 가능하나 정확한 제목은 식별하지 못함"을 1문장 언급할 수 있습니다.
...
- visualConfidence ≥ 80: assert the artist. possibleCandidates may be [] or secondary suggestions.
- visualConfidence 50–79: artist holds your most likely candidate, possibleCandidates lists alternatives.
- visualConfidence < 50: artist=null, possibleCandidates=[]; focus on visual analysis only.

Title policy — stricter than artist
- title requires a higher bar than artist. "Probably by this artist" and "exactly this titled work" are different judgments.
- Fill title ONLY when you can identify the SPECIFIC named piece (typically visualConfidence ≥ 70 AND you recognize it as a known particular work by that artist).
- If you know the artist but not which specific work, set title=null. Do NOT fill with generic descriptors ("untitled", "abstract", "landscape").
- When you cannot fill title, you may note in visualReason that "the artist is recognizable but the specific work could not be identified."
