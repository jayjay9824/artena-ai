      ...
      <div style={{ display: "flex", flexDirection: "column" as const, gap: 1 }}>
        {[
          ...
          <div key={label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", background: accent ? "#7C6FF7" : "#FAFAFA", border: `1px solid ${accent ? "#7C6FF7" : "#F0F0F0"}` }}>
            <span style={{ fontSize: 9, letterSpacing: ".14em", color: accent ? "rgba(255,255,255,0.7)" : "#BBB" }}>{label}</span>
            <span style={{ fontSize: accent ? 14 : 12, fontWeight: 700, color: accent ? "#FFF" : "#333", fontFamily: "'KakaoBigSans', system-ui, sans-serif" }}>{value}</span>
          </div>
        ))}
      </div>
    </div>
...
export function MarketIntelligenceReport({ data }: { data: MarketIntelligenceData }) {
  ...
  return (
    <div style={{ background: "#FFFFFF", border: "1px solid #E0E0E0", padding: "44px 40px", fontFamily: "'KakaoSmallSans', system-ui, sans-serif" }}>
