...
import { readUserLang } from "../../services/ai/userLang";
...
export async function POST(req: NextRequest) {
  try {
    ...
    // STEP 8 — V1 quick fix: thread the user's UI language through
    // to the analyze service so the model answers (and the rejection
    // fallback localises) in their language.
    const userLang = readUserLang(req);

    if (contentType.includes("application/json")) {
      const { query } = await req.json();
      if (!query) return NextResponse.json({ success: false, error: "검색어가 없습니다" }, { status: 400 });
      const r = await analyzeFromText(query, userLang);
    ...
    const r = await analyzeFromImage(base64, mediaType, userLang);
    if (r.kind === "rejected") return NextResponse.json({ success: false, error: r.reason }, { status: 422 });
    return NextResponse.json({ success: true, data: r.data });
